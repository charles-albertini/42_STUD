...
/*   Updated: 2025/03/25 18:53:22 by calberti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	verif_move(t_config *config, double new_y, double new_x)
{
	...
	if (new_y <= 0.5 || new_y >= config->map.height - 1
		|| new_x <= 0.5 || new_x >= config->map.width - 1)
		return (0);
	if (config->map.grid[(int)new_y][(int)new_x] == '1'
		|| config->map.grid[(int)new_y][(int)new_x] == 'O')
		return (0);
	if (config->map.grid[(int)new_y][(int)new_x] == 'V'
		&& config->map.grid[(int)new_y + 1][(int)new_x] == '1'
		&& config->map.grid[(int)new_y][(int)new_x + 1] == '1'
		&& config->map.grid[(int)new_y][(int)new_x] != 'O')
		return (0);
	return (1);
}
